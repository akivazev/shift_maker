...
use crate::util::{input_time, is_valid_time};
...
fn main() {
    ...
    let (shift_list, _) = interval_fixed_shift_list(&names, shift_length, start_time, end_time);

    for (name, time) in shift_list {
        println!("{}-{} {}", time, time + shift_length, name);
    }
}
